...
#####################################################################
HEADERS := $(wildcard *.h)
SRC := $(filter-out wasm_api.c test_stc12.c test_blink.c test_adc.c, $(wildcard *.c))
OBJ := $(SRC:.c=.o)
...
test: test_stc12 test_blink test_adc test-images
	@echo "=== Unit tests ==="
	./test_stc12
	@echo ""
...
clean:
	-rm -f $(BIN) $(OBJ) test_stc12 test_blink test_adc
